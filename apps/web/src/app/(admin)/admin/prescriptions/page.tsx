...
export default function AdminPrescriptionsPage() {
  ...
  return (
    <div>
      <h2 className="text-[24px] font-bold text-neutral-dark mb-6">Resep</h2>
      ...
      <div className="bg-white border border-neutral-border rounded-card shadow-card overflow-hidden">
        <table className="w-full text-[14px]">
          ...
          <tbody className="divide-y divide-neutral-border">
            {isLoading && (
              <tr><td colSpan={5} className="text-center py-8 text-neutral-muted">Memuat...</td></tr>
            )}
            {!isLoading && (!data?.data || data.data.length === 0) && (
              <tr><td colSpan={5} className="text-center py-8 text-neutral-muted">Belum ada resep</td></tr>
            )}
            {data?.data.map((rx) => (
              <tr key={rx.id} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-neutral-dark">{rx.patientName}</td>
                <td className="px-4 py-3 text-neutral-muted">dr. {rx.doctorName}</td>
